...
void RTC_Get_Time(void)
{
	...
	temp=RTC->TR;
	UTCTimer.hour=RTC_BCD2DEC((temp>>16)&0X3F);
	UTCTimer.minute=RTC_BCD2DEC((temp>>8)&0X7F);
	UTCTimer.second=RTC_BCD2DEC(temp&0X7F);
	//*ampm=temp>>22; 
}
//获取RTC日期
//*year,*mon,*date:年,月,日
...
void RTC_Get_Date(void)
{
	u32 temp=0;
 	while(RTC_Wait_Synchro());	//等待同步  	 
	temp=RTC->DR;
	UTCTimer.year=RTC_BCD2DEC((temp>>16)&0XFF);
	UTCTimer.months=RTC_BCD2DEC((temp>>8)&0X1F);
	UTCTimer.day=RTC_BCD2DEC(temp&0X3F);
	UTCTimer.week=(temp>>13)&0X07; 
}
//RTC初始化
//返回值:0,初始化成功;
//       1,LSE开启失败;
...
u8 RTC_Init(void)
{
	...
	if(RTC_Read_BKR(0)!=0X5050)		//是否第一次配置?
	{
		...
		RTC->ISR&=~(1<<7);			//退出RTC初始化模式
		RTC->WPR=0xFF;				//使能RTC寄存器写保护  
		RTC_Set_Time(15,04,00,0);	//设置时间
		RTC_Set_Date(24,12,21,6);		//设置日期
