...
//mode:0,普通模式;1,回环模式;
//Fpclk1的时钟在初始化的时候设置为42M,如果设置CAN2_Mode_Init(1,6,7,6,1);
...
{	
	u8 mbox;
	u16 i=0;	  	 						       
    mbox=CAN2_Tx_Msg(0X12,0,0,len,msg);
	while((CAN2_Tx_Staus(mbox)!=0X07)&&(i<0XFFF))i++;//等待发送结束
...
{		   		   
	u32 id;
	u8 ide,rtr,len; 
	if(CAN2_Msg_Pend(0)==0)return 0;		//没有接收到数据,直接退出 	 
  	CAN2_Rx_Msg(0,&id,&ide,&rtr,&len,buf); 	//读取数据
